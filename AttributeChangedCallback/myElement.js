...
class myElement extends HTMLElement {
    ...
    //3er Ciclo de vida de un web component: attributeChangedCallback
    //Es estar observando los cambios a los atributos del componente
    //Se divide en 2 partes: 1) poner un observer 2) monitorear

    //paso 1) Crear el observer, este nos permite saber que 
    //atributos van hacer monitoreados por el componente
    //si hay otros atributos adicionales no los pela (no los observa)
    static get observedAttributes() {
        return ["nombre","titulo", "parrafo", "img"]
    }
    //paso 2) utilizar el attributeChangedCallback
    //recibe 3 parametros: el atributo a evaluar: es el nombre del atributo que recibe
    //, el viejo valor solo existe si ya hay un nuevo valor
    /*
    ...
    */
    ...
    getTemplate() {
        ...
        template.innerHTML = `
        <section>
            <h3 class="texto"> ${this.nombre} </h3>
            <h2 class="title"> ${this.titulo} </h2>
            <div>
                <p> ${this.parrafo} </p>
                <!--img src="${this.imagen}" alt="metodo normal"/-->
                <img src="${this.img}" alt="metodo optimizado"/>
            </div>
        </section>
        ${this.getStyles()}
        `;
